...
export const postApplication = catchAsyncError(async (req, res, next) => {
  ...
  if (role === "Employer") {
    return next(
      new ErrorHandler(" Employer is not allowed to access this field!", 400)
    );
  }
  ...
  if (!allowedFormats.includes(resume.mimetype)) {
    return next(
      new ErrorHandler(
        "Invalid file type. Please upload your resume in JPG OR JPEG Format.",
        400
      )
    );
  }
...
export const employerGetAllApplications = catchAsyncError(
  async (req, res, next) => {
    ...
    if (role === "Job Seeker") {
      return next(
        new ErrorHandler(
          " Job Seeker is not allowed to access this field!",
          400
        )
      );
    }
